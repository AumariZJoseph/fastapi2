...
async def ingest_document_background(
    ...
    """Background document ingestion endpoint"""
    # ✅ Check rate limit for file operations
    is_limited, message = rate_limiter.is_rate_limited(user_id, "file_operations")
    if is_limited:
        raise HTTPException(status_code=429, detail=message)
    ...
    try:
        ...
        try:
            ...
            result = await fast_ingest_service.ingest_file_background(sanitized_user_id, file)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error processing file in background: {str(e)}"
        )